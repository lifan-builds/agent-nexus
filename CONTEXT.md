...
## Project
`agent-nexus` is the safe package manager for serious agent workspaces: it installs MCP servers, skills, hooks, and GitHub agent packages across AI IDEs (Claude Code, Cursor, Google Antigravity, Codex) from one personal manifest. The CLI (`nexus.py`) handles the full lifecycle: initialize a personal manifest, fetch packages from GitHub, auto-discover all asset types, deduplicate hooks, security-review MCP configs, preserve local MCP secrets during merge, deploy native target config, write a traceable lockfile, and verify with doctor. Public positioning should emphasize package-oriented safety and traceability rather than broadest target count or generic one-YAML sync: AGHub is the breadth competitor, GAAL is the polished machine-sync/onboarding competitor, and Nexus should win on dry-run review, package snapshots, overlays, lockfiles, and doctor verification. Tech stack: Python 3.10+ single-file CLI, PyYAML only, YAML manifests, Git shallow clones. Go rewrite (v1.0) is a later milestone, not required for release readiness.
