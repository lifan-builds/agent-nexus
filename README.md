...
You need Python 3.10+, Git, PyYAML, and Node.js if any MCP server uses `npx`.
...
The installer creates a reversible symlink at `~/.local/bin/nexus` and refuses to replace an unrelated command unless you pass `--force`. If `~/.local/bin` is not on `PATH`, keep using `python nexus.py ...` from the checkout.
...
`nexus init` creates `nexus.personal.yml` from the example manifest. Use the personal manifest for machine-specific setup; commit `nexus.yml` only when you want a shared team stack.
...
A real sync asks for approval before applying MCP changes unless you pass `--yes`.
